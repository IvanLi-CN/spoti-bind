...
- 如果 macOS 禁用了事件 Tap，SpotiBind 会重试一次；恢复窗口内再次失败时，会关闭转发并将模式设为 **关闭**。
...
打开 **系统设置 → 隐私与安全性 → 辅助功能**，必要时先添加 SpotiBind，再启用它。返回 SpotiBind 后，如果状态还没有刷新，可以先将当前模式切换为 **关闭**，再切回原模式；应用也会定期重新检查就绪状态。
