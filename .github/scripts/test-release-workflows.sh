...
grep -q "object.sha // empty" .github/workflows/cask-release-sync.yml
...
grep -q 'tag_ref="$(gh api "repos/${GITHUB_REPOSITORY}/git/ref/tags/${TAG}" 2>/dev/null || true)"' .github/workflows/release.yml
grep -q "jq -r '.object.sha // empty'" .github/workflows/release.yml
! grep -q 'ref="$(gh api "repos/${GITHUB_REPOSITORY}/git/ref/tags/${TAG}" --jq' .github/workflows/release.yml
